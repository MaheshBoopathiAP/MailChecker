...
function ExcelCsvUploader() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // --- 🔹 Generate filename dynamically on frontend ---
      const date = new Date();
      const formattedDate = date.toISOString().split("T")[0]; // yyyy-mm-dd

      // Optional: include time for more uniqueness (uncomment next line)
      // const formattedTime = date.toTimeString().split(" ")[0].replace(/:/g, "-");

      // Track sequential downloads using localStorage
      let fileCount = parseInt(localStorage.getItem("fileDownloadCount") || "0", 10);
      fileCount += 1;
      localStorage.setItem("fileDownloadCount", fileCount);

      // Construct dynamic filename
      const downloadFileName = `filtered_output_${formattedDate}_${fileCount}.xlsx`;
      // If you included time above, use:
      // const downloadFileName = `filtered_output_${formattedDate}_${formattedTime}_${fileCount}.xlsx`;
      ...
      setSuccessMsg(`File sent successfully! Downloaded as ${downloadFileName}`);
    } catch (error) {
